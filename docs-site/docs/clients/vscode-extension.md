...
> 技术实现详见 [TECH.md §19](/developer/architecture#19-vs-code-扩展架构)。
...
也可直接使用 [`publish-vscode.sh`](https://github.com/ChinaCarlos/fund-helper/blob/main/publish-vscode.sh)（与桌面端 [`publish-desktop.sh`](https://github.com/ChinaCarlos/fund-helper/blob/main/publish-desktop.sh) 用法类似）。不传版本号时自动读取 `versions.json`。
...
- 根目录 [`.vscode/launch.json`](https://github.com/ChinaCarlos/fund-helper/blob/main/.vscode/launch.json) — 从 monorepo 根 F5
- 子目录 [`vscode-extension/.vscode/launch.json`](https://github.com/ChinaCarlos/fund-helper/blob/main/.vscode/launch.json) — 单独打开 `vscode-extension` 时 F5
...
| 资源 | 链接 |
|------|------|
| 架构详解 | [TECH.md §19](/developer/architecture#19-vs-code-扩展架构) |
| 浏览器插件（UI 同源） | [chrome-extension/README.md](/clients/chrome-extension) |
| 养基宝 API | [API_README.md](/developer/yjb-api) |
