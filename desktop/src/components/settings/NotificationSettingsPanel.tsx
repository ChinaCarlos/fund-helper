...
import {
  ...
  loadNotificationConfigFromStorage,
      ...
      <div className="channel-connectivity">
        {connectivity.message ? (
          <p
            ...
            style={{
              margin: 0,
              color: connectivity.status === 'error' ? 'var(--destructive)' : undefined,
            }}
          >
            {connectivity.message}
          </p>
...
function DeliveryTargetPanel({
  ...
  const loadChats = useCallback(async () => {
    ...
    try {
      ...
      const result = await api.listDeliveryTargets(channel, JSON.stringify(config));
...
function FeishuDeliveryTargetPanel({
  ...
  const loadChats = useCallback(
    async (options?: { hint?: string; delayMs?: number }) => {
      ...
      try {
        const config = mergeNotificationConfig(form.getFieldsValue(true));
        const result = await api.listDeliveryTargets('feishu', JSON.stringify(config));
        setChatOptions(result.chats);
        setLoadMessage(result.message);
      } catch (err) {
        setChatOptions([]);
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      const config = mergeNotificationConfig(form.getFieldsValue(true));
      const result = await api.createFeishuNotificationGroup(JSON.stringify(config), {
        mobile,
        groupName,
      });
  ...
  return (
    <div>
      ...
      <Alert
        ...
        style={{ marginTop: 16 }}
        message="推送说明"
        description="可多选消息类型。配置保存在本地；定时推送功能将在后续版本接入。"
      />
    </div>
  );
}
...
export function NotificationSettingsPanel({ onSaved }: NotificationSettingsPanelProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      const merged = disableDeferredAppModes(await loadNotificationConfigFromStorage());
      if (!cancelled) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await persistNotificationConfig(settings);
