...
export function getWebviewHtml(webview: vscode.Webview, extensionUri: vscode.Uri): string {
  ...
  html = html.replace(/(href|src)="([^"]+)"/g, (_match, attr: string, url: string) => {
    ...
    const resource = webview.asWebviewUri(vscode.Uri.joinPath(webviewDist, clean));
...
export class FundHelperController {
  ...
  private async handleMessage(webview: vscode.Webview, msg: WebviewInbound): Promise<void> {
    switch (msg.type) {
      case 'boot':
  ...
  private async handlePollQr(webview: vscode.Webview, qrId: string): Promise<void> {
    try {
...
export async function revealFundHelperBottomPanel(): Promise<void> {
  ...
  await vscode.commands.executeCommand('workbench.view.extension.fund-helper-panel');
  try {
