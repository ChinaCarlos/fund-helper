...
export function findPointIndex(points: IncomeLinePoint[], targetTime: string): number {
  const target = parseTimeLabel(
    targetTime.includes(':') && targetTime.split(':').length >= 3
      ? targetTime
      : `${targetTime}:00`,
  );
  let best = 0;
  ...
  for (let i = 0; i < points.length; i++) {
    const dist = Math.abs(parseTimeLabel(points[i].label) - target);
    if (dist < bestDist) {
      bestDist = dist;
      best = i;
    }
...
export function getSessionXLabels(
  ...
  const end = formatTimeLabel(points.at(-1)!.label);
...
export function getSessionTimeRange(points: IncomeLinePoint[]): string {
  ...
  return `${formatTimeLabel(points[0].label)}–${formatTimeLabel(points.at(-1)!.label)}`;
}
...
export function buildIncomeChart(
  ...
  const areaPath = `${linePath} L ${coords.at(-1)?.x ?? pad.left} ${baselineY} L ${coords[0]?.x ?? pad.left} ${baselineY} Z`;
