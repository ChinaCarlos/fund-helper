...
export function PortfolioView({
  ...
  const activeAccount = snapshot.accounts.find((a) => String(a.account_id) === activeTab);
    ...
    <div className="portfolio-shell">
      ...
      <header className="header header-compact">
        <div className="user-row">
          {avatarUrl ? (
            ...
            <span className="avatar avatar-fallback">{session.nickname?.[0] ?? '基'}</span>
          )}
      ...
      <div className="summary-grid">
        <div className="summary-card">
          ...
          <span className="summary-value">{formatMoney(snapshot.total_assets)}</span>
        </div>
        <div className="summary-card">
          <span className="summary-label">当日收益</span>
          <span className={`summary-value ${colorClass(snapshot.today_income)}`}>
            {formatSigned(snapshot.today_income)}
          </span>
          <span className={`summary-sub ${colorClass(snapshot.today_income_rate)}`}>
        ...
        <div className="sort-bar">
          ...
          {sortOptions.map((key) => (
            ...
            >
              ...
              {sortKey === key ? (
                <span className="sort-order">{sortOrderSymbol(sortOrder)}</span>
              ) : null}
            </button>
          ))}
        </div>
        ...
        {sortedFunds.length === 0 ? (
          ...
          sortedFunds.map((fund) => {
            ...
            <div key={`${fund.account_id}-${fund.code}`} className="fund-row">
              <div>
                <div className="fund-name">{fund.short_name || fund.code}</div>
