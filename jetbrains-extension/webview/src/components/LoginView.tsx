...
import { postToExtension, useExtensionMessage } from '@/bridge';
...
export function LoginView() {
  ...
  const statusRef = useRef<LoginState>('loading');
  ...
  useExtensionMessage((msg: { type: string; id?: string; url?: string; result?: QrStateResult; message?: string }) => {
    if (msg.type === 'qr' && msg.id && msg.url) {
      ...
      void drawQr(msg.url).then(() => {
        const canvas = canvasRef.current;
        if (canvas) canvas.dataset.qrUrl = msg.url;
        setStatus('waiting');
        statusRef.current = 'waiting';
        setMessage('请使用微信扫一扫，并在手机上确认授权');
        postToExtension({ type: 'pollQr', qrId: msg.id });
      });
  ...
  useEffect(() => {
    startLogin();
    const onTheme = () => {
      if (qrIdRef.current && statusRef.current === 'waiting') {
        const canvas = canvasRef.current;
        const url = canvas?.dataset.qrUrl;
        if (url) void drawQr(url);
      }
    };
    window.addEventListener('jetbrains-theme', onTheme);
    return () => {
      runIdRef.current += 1;
      clearTimers();
      window.removeEventListener('jetbrains-theme', onTheme);
    };
  }, [startLogin, clearTimers, drawQr]);
